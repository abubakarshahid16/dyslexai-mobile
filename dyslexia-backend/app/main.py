...
from app.database import engine, Base, IS_SQLITE, get_db
import app.models
...
@app.get("/health")
def health_check():
    return {"status": "ok", "service": "dyslexia-backend"}
...
def health_db_check(db: Session = Depends(get_db)):
    try:
        ...
        # Check if game curriculum is seeded (Day 1 exists)
        seeded = db.query(GameDay).filter(GameDay.day_number == 1).first() is not None
        return {"status": "ok", "database": "connected", "exercises_seeded": seeded}
    except Exception as e:
        return {"status": "error", "detail": str(e), "exercises_seeded": False}
